...
performs fold changes analysis for each pair of levels of a desired
variable and creates a new table.
...
- f:
  ...
  factor variable considered for performing the Fold Change analysis.
...
- only_on_positive:
  ...
  logical. If TRUE, FC analysis will be performed only if all values are
  positive (if paired), or if all means are positive (if not paired).
  The rest will be left as NA.

## Value

A tibble the results of the Fold Change analysis for each pair of levels
